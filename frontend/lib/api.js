...
export const api = {
  ...
  signup: (email, password, name, referralCode) => request('/auth/signup', { method: 'POST', body: JSON.stringify({ email, password, name, referralCode }) }),
  // two-step OTP signup
  ...
  assetsAll: () => request('/assets?all=1'),
  paymentMethods: (kind) => request(`/payment-methods${kind ? `?kind=${kind}` : ''}`),
  ...
  // Network Compensation — user
  networkMe: () => request('/network/me'),
  networkLevels: () => request('/network/levels'),
  networkTransactions: (type) => request(`/network/transactions${type ? `?type=${type}` : ''}`),
  networkTeam: () => request('/network/team'),

  // Network Compensation — admin
  adminNetworkLevels: () => request('/admin/network/levels'),
  adminCreateNetworkLevel: (body) => request('/admin/network/levels', { method: 'POST', body: JSON.stringify(body) }),
  adminUpdateNetworkLevel: (id, body) => request(`/admin/network/levels/${id}`, { method: 'PUT', body: JSON.stringify(body) }),
  adminDeleteNetworkLevel: (id) => request(`/admin/network/levels/${id}`, { method: 'DELETE' }),
  adminReorderNetworkLevels: (ids) => request('/admin/network/levels/reorder', { method: 'POST', body: JSON.stringify({ ids }) }),
  adminNetworkMembers: (q = '') => request(`/admin/network/members${q ? `?q=${encodeURIComponent(q)}` : ''}`),
  adminNetworkRecalc: (userId) => request('/admin/network/recalc', { method: 'POST', body: JSON.stringify({ userId }) }),
  adminNetworkSalaryRun: (force = false) => request('/admin/network/salary/run', { method: 'POST', body: JSON.stringify({ force }) }),
  adminSetSponsor: (userId, { sponsorId, sponsorCode }) => request(`/admin/network/sponsor/${userId}`, { method: 'POST', body: JSON.stringify({ sponsorId, sponsorCode }) }),
  adminNetworkTransactions: (params = {}) => {
    const q = new URLSearchParams(params).toString();
    return request(`/admin/network/transactions${q ? `?${q}` : ''}`);
  },
};
